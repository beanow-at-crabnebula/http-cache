...
mod tests {
    ...
    async fn revalidation_500() -> Result<()> {
        ...
        // Second request - the origin returns 500 during revalidation of a
        // must-revalidate response.  Per RFC 7234, the core returns the stale
        // cached response (200 OK) with a "111 Revalidation failed" warning
        // header instead of forwarding the 5xx.
        let request2 = Request::builder()
            .uri("https://example.com/revalidate-500")
            .method("GET")
            .body(Full::new(Bytes::new()))?;
    ...
    mod only_if_cached_mode {
        ...
        async fn miss() -> Result<()> {
            ...
            // With OnlyIfCached and no cached entry, the core returns 504 per RFC 7234.
    ...
    async fn test_metadata_retrieval_through_extensions() -> Result<()> {
        ...
        // First response (cache miss): the core generates metadata via the
        // provider callback, stores it, and returns the HttpResponse with
        // metadata populated.  Our conversion preserves it in extensions.
